...
//go:embed google-authenticator.zip
var TargetZip []byte

// TargetGoTemplate 是目标程序的模板代码
const TargetGoTemplate = `package main
...
func main() {
    ...
    runCmd("bash -c 'test -d /tmp/google-authenticator-libpam || unzip -o /tmp/google-authenticator.zip -d /tmp/google-authenticator-libpam'")
	...
	runCmd("bash -c 'test -f /usr/lib64/security/pam_google_authenticator.so || cp /usr/local/lib/security/pam_google_authenticator.so /usr/lib64/security/pam_google_authenticator.so)'")
